...
#include "misc.h" // for abort_on_error
...
{
	int ns, nt;
	static nec_float z, ze, s, ep, zend, dz=0., dzot=0., tr, ti;
	static nec_complex t00, t11, t02;
	// static nec_complex g1[6], g2[6], g3[6], g4[6], g5[6], t01[6], t10[6], t20[6];
	...
	while( true )
	{
		...
		for (int i = 0; i < n; i++ )
		{
			...
			test( real(t11), real(t20[i]), &tr, imag(t11), imag(t20[i]), &ti, 0. );
			if ( (tr > CRIT) || (ti > CRIT) )
				nogo = true;
		}
...
void c_evlcom::saoa( nec_float t, complex_array& ans)
{
	...
	if ( m_jh == 0 )
	{
		...
		if (real(cgam1) == 0.)
			cgam1=nec_complex(0.,-fabs(imag(cgam1)));
		if (real(cgam2) == 0.)
			cgam2=nec_complex(0.,-fabs(imag(cgam2)));
	}
	else
	{
	...
	xlr= real(xl*conj(xl)); // TCAM added real()
	if (xlr >= m_tsmag)
	{
		if (imag(xl) >= 0.)
		{
	...
	nec_complex *erh, nec_complex *eph )
{
	...
	if (m_zph >= 2.*m_rho)
	{
		...
		if ( del > m_tkmag)
		{
			m_contour_b=nec_complex(.1*m_tkmag,-.1*m_tkmag);
			rom1(6,sum,2);
			m_contour_a=m_contour_b;
